...
export class AutoLinkSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    // new Setting(containerEl)
    //   .setName("Manual scan hotkey")
    //   .setDesc(
    //     "Assign a hotkey to 'Auto-link: Scan whole vault' in Settings → Hotkeys. The ribbon icon and command palette also trigger it.",
    //   );
    ...
    if (s.masterlistEnabled) {
      new Setting(containerEl)
        ...
        .addSearch((search) => {
          ...
          new MarkdownFileSuggest(this.app, search.inputEl, async (path) => {
